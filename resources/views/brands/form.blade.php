<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('brands.index') }}" class="text-sm text-gray-600 underline">{{ __('Back to brands') }}</a>
        </div>
    </x-slot>

    @php
        // New records default to active (matches the is_active column default).
        // The slug is generated from the name by the Brand model.
    ...
    <div class="py-12 max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
        ...
        <form method="POST" action="{{ $submitUrl }}" class="bg-white shadow rounded-lg p-6 space-y-6">
            ...
            <div class="flex items-center gap-3">
                <x-primary-button>{{ $brand->exists ? __('Update brand') : __('Create brand') }}</x-primary-button>
                <a href="{{ route('brands.index') }}" class="text-sm text-gray-600 underline">{{ __('Cancel') }}</a>
            </div>
        </form>
    </div>
</x-app-layout>